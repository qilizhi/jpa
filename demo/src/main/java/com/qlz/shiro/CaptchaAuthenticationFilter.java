...
public class CaptchaAuthenticationFilter extends FormAuthenticationFilter {
	...
	// 允许登录错误次数，当登录次数大于该数值时，会在页面中显示验证�?
	private Integer allowIncorrectNumber = 3;

	/**
	 * 重写父类方法，在shiro执行登录时先对比验证码，正确后在登录，否则直接登录失�?
	 */
	@Override
	protected boolean executeLogin(ServletRequest request,
		...
		// 如果登录次数大于allowIncorrectNumber，需要判断验证码是否�?��
	...
	/**
	 * 重写父类方法，当登录失败将异常信息设置到request的attribute�?
	 */
	@Override
	...
	/**
	 * 重写父类方法，当登录成功后，将allowIncorrectNumber（允许登错误录次）设置为0，重置下�?��登录的状�?
	 */
	@Override
	...
	/**
	 * 创建�?��shiro的session,如果存在session就用现有的session，否则创建一个新的session
	 * 
	 * @return {@link Session}
	 */
	...
	/**
	 * 设置允许登录次数，当登录次数大于该数值时，会在页面中显示验证�?
	 * 
